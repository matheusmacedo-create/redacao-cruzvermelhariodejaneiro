...
export default async function EditarParticipante({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (nivel < 2) redirect(`/participantes/${id}`)
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-4">
      <Link href={`/participantes/${id}`} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ChevronLeft className="size-4" />{p.nome}</Link>
