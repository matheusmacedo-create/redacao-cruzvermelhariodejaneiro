...
export default async function ReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: rawContent }, { data: rawRequester }, { data: rawVoters }] = await Promise.all([
    ...
    supabase.from('profiles').select('id,full_name,initials,color,job_title').eq('id', approval.requested_by).maybeSingle(),
  ...
  const [{ data: pauta }, { data: profiles }] = await Promise.all([
    ...
    voterIds.length ? supabase.from('profiles').select('id,full_name,initials,color,job_title').in('id', voterIds) : Promise.resolve({ data: [] }),
  ])
  const profileById = new Map((profiles ?? []).map((profile) => [profile.id, profile]))
  const voters = (rawVoters ?? []).map((voter) => { const profile = profileById.get(voter.user_id); return { userId: voter.user_id, decision: voter.decision, comment: voter.comment, decidedAt: voter.decided_at, name: profile?.full_name || 'Colaborador', initials: profile?.initials || '?', color: profile?.color, role: profile?.job_title || 'Equipe' } })
