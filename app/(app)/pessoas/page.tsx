...
export default async function PessoasPage() {
  ...
  const { data: members } = await supabase.from('workspace_members').select('role,coordination,profiles(id,full_name,job_title,initials,color,active)').eq('workspace_id', context.workspace.id)
  const people = (members ?? []).filter((item: any) => { const p = Array.isArray(item.profiles) ? item.profiles[0] : item.profiles; return p?.active !== false }).map((item: any) => { const p = Array.isArray(item.profiles) ? item.profiles[0] : item.profiles; return { id:p.id,name:p.full_name,role:p.job_title,initials:p.initials,color:p.color,coordination:item.coordination,accessRole:item.role } })
