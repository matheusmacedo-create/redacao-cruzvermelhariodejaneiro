...
export default async function ColetaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: c } = await supabase.from('imagem_coletas')
    .select('id, titulo, descricao, token, file_ids, criado_por, created_at, expira_em, encerrada_em, profiles!imagem_coletas_criado_por_fkey(full_name)')
  ...
  const [linhas, { data: arquivos }] = await Promise.all([
    ...
    supabase.from('files').select('id, name, file_type, storage_path, authorization_status').eq('workspace_id', context.workspace.id).in('id', c.file_ids as string[]),
  ])
  ...
  const podeMexer = c.criado_por === context.user.id || pode(context.role, 'biblioteca.apagar_de_outros')
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description={`Criado ${autor?.full_name ? `por ${autor.full_name} ` : ''}em ${quando(c.created_at as string)} · ${c.expira_em ? `vale até ${quando(c.expira_em as string)}` : 'sem prazo'}`}
        breadcrumbs={[{ label: 'Biblioteca de mídia', href: '/biblioteca' }, { label: 'Autorizações de imagem', href: '/biblioteca/autorizacoes' }, { label: c.titulo as string }]}
      ...
      <section aria-labelledby="fotos" className="flex flex-col gap-3">
        <h2 id="fotos" className="text-base font-semibold">Fotos ({(arquivos ?? []).length}){pendentes > 0 && <span className="ml-2 text-sm font-normal text-muted-foreground">{pendentes} ainda como “Falta autorizar” na Biblioteca</span>}</h2>
        <ul className="grid grid-cols-4 gap-2 sm:grid-cols-6 lg:grid-cols-8">
          {(arquivos ?? []).map((a) => (
            <li key={a.id as string} className="aspect-square overflow-hidden rounded-lg bg-muted" title={a.name as string}>
              {a.file_type === 'foto' && a.storage_path
                ? <img src={`/api/private-blob?pathname=${encodeURIComponent(a.storage_path as string)}`} alt={a.name as string} loading="lazy" className="size-full object-cover" />
                : <span className="flex size-full items-center justify-center p-1 text-center text-[10px] text-muted-foreground">{a.name as string}</span>}
            </li>
          ))}
        </ul>
      </section>
