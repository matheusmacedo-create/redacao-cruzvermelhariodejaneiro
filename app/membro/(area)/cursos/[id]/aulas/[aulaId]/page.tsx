  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
      <div className="flex min-w-0 flex-col gap-4">
        ...
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Aula {posicao} de {d.emOrdem.length}</p>
          <h1 className="text-2xl font-bold tracking-tight">{aula.titulo}</h1>
        </div>
        {aula.texto && <div className="whitespace-pre-line rounded-2xl border border-neutral-200 bg-white p-5 leading-relaxed text-neutral-800" id="texto-da-aula">{aula.texto}</div>}
        {aula.material_id && (
          <a href={`/membro/apostilas/${aula.material_id}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-4 text-sm hover:border-neutral-300">
            <FileDown className="size-5 text-[#e32219]" /><span className="font-medium">Abrir a apostila desta aula</span>
          </a>
        )}
        <div className="flex items-center justify-between gap-3 border-t border-neutral-200 pt-4">
          {anterior ? <Link href={`/membro/cursos/${id}/aulas/${anterior}`} className="text-sm font-medium text-neutral-600 hover:text-neutral-900">← Anterior</Link> : <span />}
      ...
      <aside className="flex flex-col gap-3 lg:sticky lg:top-20 lg:self-start">
        ...
        <nav className="max-h-[70vh] overflow-y-auto rounded-2xl border border-neutral-200 bg-white" aria-label="Aulas do curso">
          {d.modulos.map((mo) => (
            <div key={mo.id}>
              <p className="sticky top-0 border-b border-neutral-100 bg-neutral-50 px-3 py-2 text-xs font-semibold text-neutral-600">{mo.titulo}</p>
              <ul>
                {mo.aulas.map((a) => (
                  <li key={a.id}>
                    <Link href={`/membro/cursos/${id}/aulas/${a.id}`} aria-current={a.id === aulaId ? 'page' : undefined}
                      className={`flex items-center gap-2 px-3 py-2 text-sm ${a.id === aulaId ? 'bg-red-50 font-medium text-[#e32219]' : 'hover:bg-neutral-50'}`}>
                      {a.feita ? <CheckCircle2 className="size-4 shrink-0 text-emerald-600" /> : <Circle className="size-4 shrink-0 text-neutral-300" />}
