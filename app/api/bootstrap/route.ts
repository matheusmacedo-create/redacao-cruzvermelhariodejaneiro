...
export async function POST(request: Request) {
  ...
  if (demo) {
    ...
    await admin.from('inbox_items').insert([{ workspace_id: demo.id, type: 'request', title: 'Divulgação da campanha de doação de sangue', summary: 'Material recebido para triagem editorial.', sender_name: 'Coordenação de Saúde', coordination: 'Saúde', priority: 'high', status: 'new', created_by: userId }])
    await admin.from('content_pieces').insert({ workspace_id: demo.id, pauta_id: pauta?.id, title: 'Voluntários levam cuidado ao Centro do Rio', format: 'article', status: 'draft', body: 'Uma ação da Cruz Vermelha Brasileira reuniu voluntários no Centro do Rio de Janeiro.\n\nA equipe realizou orientações de saúde e distribuiu kits de higiene para a população.', responsible_id: userId, created_by: userId })
    await admin.from('calendar_events').insert({ workspace_id: demo.id, title: 'Publicação da ação no Centro', event_date: '2026-08-28', event_time: '10:00', type: 'publication', pauta_id: pauta?.id, created_by: userId })
  }
