...
export function Topbar({ workspace, role, profile, notifications }: { workspace: { name: string; kind: 'demo' | 'production' }; role: WorkspaceRole; profile: any; notifications: Notification[] }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex min-h-14 shrink-0 items-center gap-2 border-b border-border bg-background/95 px-2 py-2 backdrop-blur-md [padding-top:max(0.5rem,env(safe-area-inset-top))] sm:min-h-16 sm:gap-4 sm:px-6">
      ...
      <Link href="/espacos" className="flex h-11 min-w-0 max-w-[42vw] items-center gap-1.5 rounded-lg border border-border px-2 text-sm transition-colors hover:bg-muted sm:max-w-xs sm:gap-2 sm:px-3"><span className={cn('size-2 shrink-0 rounded-full', workspace.kind === 'demo' ? 'bg-warning' : 'bg-success')}/><span className="truncate font-medium">{workspace.name}</span><span className="hidden rounded bg-muted px-1.5 py-0.5 text-[10px] capitalize text-muted-foreground sm:inline">{role}</span><ChevronsUpDown className="size-3.5 shrink-0 text-muted-foreground"/></Link>
      ...
      <div className="ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
        <Button size="lg" className="size-11 px-0 sm:w-auto sm:px-4" render={<Link href="/registrar" aria-label="Nova pauta"/>}><Plus className="size-4"/><span className="hidden sm:inline">Nova pauta</span></Button>
