...
import { buscarNaAjuda, hrefDaAjuda, TOPICOS_GERAIS } from '@/lib/ajuda'
...
function Conteudo() {
  ...
  const { daTela, fecharPainel, reverBoasVindas, pessoa } = useAjuda()
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4">
        {buscando ? (
          ...
          <div className="flex flex-col gap-7">
            <PrimeirosPassos />
            {daTela?.guia ? <AjudaDaArea /> : <AjudaGeral />}
          </div>
        )}
      </div>
...
function AjudaDaArea() {
  const pathname = usePathname()
  const { daTela, iniciarTour, fecharPainel } = useAjuda()
    ...
    <>
      <section aria-labelledby={`${id}-area`} className="flex flex-col gap-3">
        ...
        {daTela.tour.length > 0 ? (
          <Button type="button" size="lg" className="h-11 self-start sm:h-10" onClick={() => iniciarTour()}>
