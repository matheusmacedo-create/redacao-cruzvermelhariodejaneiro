...
export function ConcluirAula({ cursoId, aulaId, feita, seguinte }: { cursoId: string; aulaId: string; feita: boolean; seguinte: string | null }) {
  ...
  if (certificado) {
    return (
      <div className="flex flex-col gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-900" role="status" id="certificado-emitido">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href={`/membro/cursos/${cursoId}`} className="rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-emerald-100">Voltar ao curso</Link>
        </div>
      </div>
    )
  }
