...
const Selo = ({ n }: { n: number }) => (
  <span className="absolute -right-1.5 -top-1 min-w-4 rounded-full bg-[#e32219] px-1 text-center text-[10px] font-bold leading-4 text-white" aria-label={`${n} não lidas`}>{n > 9 ? '9+' : n}</span>
)
      ...
      <nav className="hidden gap-1 lg:flex" aria-label="Seções">
        {SECOES.map((s) => (
          <Link key={s.href} href={s.href} aria-current={ativo(s.href) ? 'page' : undefined}
            className={`relative rounded-lg px-3 py-2 text-sm font-medium ${ativo(s.href) ? 'bg-neutral-900 text-white' : 'text-neutral-600 hover:bg-neutral-200/70 hover:text-neutral-900'}`}>
            {s.rotulo}{s.href === '/membro/mensagens' && naoLidas > 0 && <Selo n={naoLidas} />}
          </Link>
        ))}
      </nav>
      <nav className="fixed inset-x-0 bottom-0 z-20 grid border-t border-neutral-200 bg-white/95 backdrop-blur lg:hidden" style={{ gridTemplateColumns: `repeat(${NO_CELULAR.length}, 1fr)` }} aria-label="Seções">
        {NO_CELULAR.map((s) => (
          <Link key={s.href} href={s.href} aria-current={ativo(s.href) ? 'page' : undefined}
