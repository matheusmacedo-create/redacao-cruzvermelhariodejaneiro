...
import { Award, BookOpen, CalendarHeart, GraduationCap, Home, UserRound } from 'lucide-react'
...
export const SECOES = [
  ...
  { href: '/membro/certificados', rotulo: 'Certificados', icone: Award, celular: true },
...
export function Navegacao() {
  ...
  return (
    <>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 grid border-t border-neutral-200 bg-white/95 backdrop-blur sm:hidden" style={{ gridTemplateColumns: `repeat(${NO_CELULAR.length}, 1fr)` }} aria-label="Seções">
        {NO_CELULAR.map((s) => (
          <Link key={s.href} href={s.href} aria-current={ativo(s.href) ? 'page' : undefined}
