...
/**
 * O menu da conta, em todas as larguras: avatar com a foto (ou as iniciais), nome,
 ...
 * desfaz a prévia).
 */
export function MenuDaConta({ nome, email, foto = null, previa = false }: { nome: string; email: string | null; foto?: string | null; previa?: boolean }) {
    ...
    <>
      ...
      <Menu.Root onOpenChange={(aberto) => { if (aberto) ajuda?.adiantarTour() }} onOpenChangeComplete={(aberto) => {
        ...
        <Menu.Trigger ref={gatilho} aria-label={`Conta de ${nome}`} data-ajuda="membro.conta"
          ...
          <Retrato url={foto} nome={nome} className="size-9 text-sm" />
        </Menu.Trigger>
        <Menu.Portal>
            ...
            <Menu.Popup className={cn(popup, 'w-72 max-w-[calc(100vw-1rem)] p-1.5')}>
              <Menu.Group>
                <Menu.GroupLabel className="flex items-center gap-3 px-2.5 pb-2.5 pt-1.5">
                  <Retrato url={foto} nome={nome} className="size-10 text-sm" />
