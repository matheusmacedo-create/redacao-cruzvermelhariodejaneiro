    ...
    <div className="flex flex-col items-start gap-1.5 sm:items-end">
      {inscrito ? (
        <div className="flex flex-wrap items-center gap-2">
          <a href={`/membro/oportunidades/${o.id}/agenda`} className="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-medium text-foreground/85 hover:bg-muted"><CalendarPlus className="size-4" />Agenda</a>
          <button type="button" disabled={ocupado} onClick={() => { if (confirm('Cancelar sua inscrição? A vaga vai para a próxima pessoa.')) rodar(() => cancelarInscricao(o.id)) }}
            className="rounded-lg px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted hover:text-destructive">{ocupado ? <Loader2 className="size-4 animate-spin" /> : 'Cancelar inscrição'}</button>
        </div>
      ) : (
...
const MINHA: Record<string, { texto: string; classe: string; icone: typeof CheckCircle2 }> = {
  inscrito: { texto: 'Você está inscrito', classe: 'bg-success/10 text-success', icone: CheckCircle2 },
  espera: { texto: 'Você está na lista de espera', classe: 'bg-warning/15 text-warning-foreground', icone: Hourglass },
  presente: { texto: 'Presença confirmada', classe: 'bg-success/10 text-success', icone: CheckCircle2 },
  ausente: { texto: 'Ausência registrada', classe: 'bg-muted text-muted-foreground', icone: XCircle },
}
...
export function CartaoDeOportunidade({ o, agora }: { o: OportunidadeDoMembro; agora: string }) {
  ...
  return (
    <article className={`flex gap-4 rounded-xl border border-border bg-card p-4 sm:p-5 ${e === 'cancelada' ? 'opacity-70' : ''}`} data-oportunidade={o.id}>
      <div className="flex w-14 shrink-0 flex-col items-center self-start rounded-lg bg-primary py-2 text-primary-foreground" aria-hidden="true">
        <span className="text-xl font-bold leading-none">{s.dia}</span><span className="text-[11px] font-semibold tracking-wide text-primary-foreground/80">{s.mes}</span>
      </div>
      <div className="flex min-w-0 flex-1 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-medium uppercase tracking-wide text-primary">{TIPOS[o.tipo as Tipo]?.rotulo ?? o.tipo}</p>
          ...
          {o.descricao && <p className="mt-2 line-clamp-3 whitespace-pre-line text-sm text-foreground/85">{o.descricao}</p>}
          {e === 'cancelada' && <p className="mt-2 text-sm text-destructive">Cancelada{o.motivo_cancelamento ? `: ${o.motivo_cancelamento}` : '.'}</p>}
          {e === 'encerrada' && !o.minha && <p className="mt-2 text-xs text-muted-foreground">Inscrições encerradas.</p>}
