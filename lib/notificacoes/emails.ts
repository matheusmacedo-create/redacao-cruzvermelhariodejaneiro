...
const rodape = (urlBase: string) => ({
  texto: 'Você recebe estes avisos no seu e-mail de recuperação do Palácio Virtual. Para escolher o que chega por e-mail:',
...
export function emailDeNotificacao(p: {
  ...
  const blocos: Bloco[] = [
    ...
    { tipo: 'botao', rotulo: p.botao ?? 'Ver no Palácio Virtual', url: `${p.urlBase}${p.link ?? '/notificacoes'}` },
...
export function emailDeResumo(p: { urlBase: string; nome: string; itens: ItemDoResumo[]; total: number }): EmailPronto {
  ...
  const blocos: Bloco[] = [
    { tipo: 'p', texto: `Olá, ${primeiroNome(p.nome)}. Você tem ${quantos} no Palácio Virtual que ainda não viu:` },
  ...
  return montar({
    assunto: `Você tem ${quantos} no Palácio Virtual`,
    preheader: mostrados.map((i) => i.titulo).join(' · ').slice(0, 120),
    titulo: `Seu resumo do Palácio Virtual`,
    blocos,
    rodape: rodape(p.urlBase),
  })
}
