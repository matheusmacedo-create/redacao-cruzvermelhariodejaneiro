...
| `cvrj-acervo` | biblioteca e acervo da filial | as pastas do acervo, 30 dias; `entrada/` sem trava | nunca | pessoas e o script de cópia do site |
...
| trilha | `cvrj-trilha` | Vercel da Redação: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_TRILHA=cvrj-trilha` |
| acervo | `cvrj-acervo` | quem envia arquivos ao acervo (Cyberduck, rclone) e o script de cópia do site |
...
- `lib/armazenamento/r2.ts`: cliente mínimo, com a assinatura AWS SigV4 feita ali (sem SDK) —
  ...
  apagar e listar. Usado pelo espelho da trilha (`lib/auditoria/espelho.ts`).
...
- **Tela "Biblioteca" na Redação**: enviar do navegador direto para o acervo (link de uso único),
  catálogo com título, data, assunto e etiquetas, busca e prévia, e download por link temporário.
